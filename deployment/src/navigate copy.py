...
sys.path.append("/home/linden/Desktop/ViNT/visualnav-transformer/train/vint_train/models/nomad")
from nomad import DenseNetwork
import torch
            ...
            if model_params["model_type"] == "nomad":
                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        # Append the key to the list (and keep the history size limited)
                        keystrokes.append(key)
                        if len(keystrokes) > MAX_KEYSTROKES:
                            keystrokes.pop(0)  # Keep only the last MAX_KEYSTROKES keys

                # Initialize the mapper
                #goal_mapper = GoalMapperNet(input_dim=2, latent_dim=model_params["goal_encoding_size"]).to(device)

                # discrete_commands = {
                #     "left": torch.nn.Parameter(torch.randn(3, device=device)),
                #     "right": torch.nn.Parameter(torch.randn(3, device=device)),
                #     "straight": torch.nn.Parameter(torch.randn(3, device=device)),
                # }
                ...
                batch_obs_imgs = []
                batch_goal_data = []
                eye_mat = torch.eye(3).int().to(device)
                ...
                if not keystrokes:
                    ...
                    # command = "straight"
                    goal_data = eye_mat[1]
                # goal_latent = discrete_commands[command]


                # goal_data = transform_images(sg_img, model_params["image_size"]) # change to 1D tensor
                batch_obs_imgs.append(transf_obs_img)
                batch_goal_data.append(goal_data) #or goal_latent
                ...
                #print("goal tensor size" ,batch_goal_data.shape)  
